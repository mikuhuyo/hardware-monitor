  ...
  <div class="data-container">
    <div class="seachBox">
      <el-form :inline="true" :model="formInline" class="demo-form-inline">
        ...
        <div class="searchBut" style="display:inline-block" @click="onSubmit">
    ...
    <div class="contBox" v-infinite-scroll="handleCurrentChange" style="overflow:auto" v-loading="loading"
      ...
      <div v-for="(item, ind) in tableData" :class="['item', calssName(item)]" :key="ind">
        <div class="lable" v-if="item.level == 0"> <span class="hov">{{item.online == true ? '正常' : '离线'}} </span></div>
        <div class="lable" v-if="item.level != 0"> <span class="hov">{{item.level == 2 ? '一般报警' : '严重报警'}}</span></div>
        <div class="title">{{item.deviceId}}</div>
        <div class="info">
          <div v-for="(it, ind) in item.quotaList" :key="ind">
            <div v-if="it.level != 0">
              <span style="width: 600px;">{{it.name}} &nbsp;<i class="lab">{{it.value}} {{it.unit}}</i></span>
              <span style="color:#818693; width: 600px;">正常值 &nbsp; {{it.referenceValue}} {{it.unit}} </span>
            </div>
          </div>
        </div>
    ...
    <el-dialog :visible.sync="dialogVisible" width="544px" center>
      <div slot="title" class="dialog-title">
        设备指标
      </div>
      <div class="dataInfo">
        <div v-for="(it, ind) in itemData.quotaList" class="item" :key="ind">
          <span style="text-align: left; width: 600px;">{{it.name}} &nbsp;
            <i style="color:#999" :class="{lab: it.level > 0}">{{it.value}} {{it.unit}}</i>
          </span>
          <span style="color: #818693; text-align: left; width: 600px;">正常值 &nbsp; {{it.referenceValue}} {{it.unit}} </span>
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false" size="small" class="butBox">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
