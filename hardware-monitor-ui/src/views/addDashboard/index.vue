      ...
      <el-form :model="form" label-width="100px" style="padding:10px; position: relative;" ref="form" label-position="left">
        ...
        <div class="fromRow">
          <el-form-item label="选择指标" prop="quota">
            <el-select v-model="form.quota" filterable placeholder="指标（单选）" size="small" class="select-box" @change="getQuotaDevices">
              <el-option v-for="item in quotaNumberQuotaArr" :key="item.id" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
          <div class="fromCol">
            <el-select v-model="form.deviceIdList" filterable multiple placeholder="设备（多选）" size="small"
              :multiple-limit=5 class="select-box">
  ...
  export default class extends Vue {
    ...
    async onSubmit() {
      ...
      const params = { ...this.form,
        device: this.form.deviceIdList.join(',')
      }
    ...
    async getQuotaNumberQuota() {
      await quotaNumberQuota().then((res: any) => {
        this.quotaNumberQuotaArr = [...res.result];
      })
