    ...
    <div class="alarmLog-container">
      ...
      <div class="seachBox">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="告警级别: " class="taglong">
            <el-select v-model="formInline.alarm" placeholder="请选择">
              <el-option v-for="item in alarmLevelSelect" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="时间：" class="tagSearch">
            <el-date-picker v-model="timeVal" :clearable="false" type="daterange" range-separator="至"
      ...
      <div class="contBox">
        <div :class="{'item': true, 'itemLev': item.level == 1}" v-for="(item,index) in tableData" :key="index">
          ...
          <div class="itemInfo">
            ...
            <div v-else><span class="iconfont icon-shijian"></span>哈?日期时间解析失败:(</div>
            <div style="text-align: left;padding-left: 160px;">
              <span class="iconfont icon-baojing"></span>
              <span v-if="item.level > 0">{{ item.alarmName }}</span>
              <span v-if="item.level == 0">指标正常 {{ item.value }}</span>
              <span v-if="item.level == 0" class="labNormal">正常</span>
              <span v-if="item.level == 1" class="labLev">告警</span>
              <span v-if="item.level == 2" class="labLev">异常</span>
            </div>
            <div style="text-align: left;padding-left: 160px;">
              <span class="iconfont icon-shuju1"></span>
              {{ item.quotaName }}
              <span v-if="item.level > 0" class="unit">{{ item.value }} {{ item.unit }}</span>
              <span v-if="item.level == 0" style="color: #57CE9E;">{{ item.value }} {{ item.unit }}</span>
            </div>
            <div>
              <span class="iconfont icon-shebei"></span> {{ item.deviceId }}
            </div>
          </div>
        </div>
      ...
      <div class="pagination" v-show="total > 10">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
          :page-sizes="[10, 20, 30, 50]" :page-size="formInline.pageSize" :current-page="formInline.page"
