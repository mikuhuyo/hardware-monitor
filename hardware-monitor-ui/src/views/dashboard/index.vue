<template>
  <div class="dashboard-container" @click.stop="() =>  boardShow ? boardShow = false : null">
          ...
          <div class="table">
            ...
            <div class="conttent" style="height: 200px;overflow: scroll;">
              <div class="trLine" v-for="it in tableData" :key="it.name">
                  ...
                  <span v-if="it.level > 0">{{it.alarmName}}</span>
                  <span v-if="it.level == 0">正常</span>
                  <span v-if="it.level == 2" style="width:120px; flex:none"><i style="background: #FF5757;">异常</i></span>
                  <span v-if="it.level == 1" style="width:120px; flex:none"><i style="background: #FFC400;">告警</i></span>
                  <span v-if="it.level == 0" style="width:120px; flex:none"><i style="background: #57CE9E;">正常</i></span>
    ...
    <el-dialog :visible.sync="dialogVisible" width="345px" center >
      <div slot="title" class="dialogTitle" >编辑看板</div>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="addBoardHanle" size="small">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default class extends Vue {
  ...
  private mapData: any[] = [];
  // 系统模块
  private boardSystemBoard: any[] = [];
  private dialogVisible:boolean = false;
  private checkList:string[] = ['1'];
  // 非系统面板
  private unSystemBoardList: any[] = [];
  private top10Data: {} = {};
  ...
  private restTop = 0;
  // 异常趋势图数据
  private trendChartDatas: string[] = [];
  ...
  // 异常设备实时数据 今天
  async getRealTimeAlarmData(){
    // 使用定时器发起轮询
    await realTimeAlarmLog().then((response : any) => {
