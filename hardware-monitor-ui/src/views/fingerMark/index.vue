<template>
  <div class="finerMark-container">
    ...
    <div class="topContent">
      ...
      <div class="addCont">
        <el-button type="primary" style="width:110px; height:36px" size="small" @click="addItem()">
          <span class="iconfont icon-tianjia"></span>添加指标
        </el-button>
    ...
    <el-dialog :visible.sync="quotaDialogVisible" width="500px" center @close="resetForm('quota')">
      <div slot="title" class="dialog-title">
        {{ title }}
      </div>

      <el-form :rules="rules" :model="form" ref="quota" label-width="140px">
        <el-form-item prop="name" label="指标名称：">
          <el-input placeholder="请输入指标名称" v-model="form.name" size="small" maxlength='20'></el-input>
        </el-form-item>
        <el-form-item prop="unit" label="单位：">
          <el-input placeholder="请输入单位" v-model="form.unit" maxlength='10' size="small"></el-input>
        </el-form-item>
        <el-form-item prop="subject" label="报文主题：">
          <el-input placeholder="请输入报文主题" v-model="form.subject" maxlength='50' size="small"></el-input>
        </el-form-item>
        <el-form-item prop="valueType" label="指标值数据类型：">
          <el-select placeholder="请选择指标值数据类型" v-model="form.valueType" size="small">
            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="valueKey" label="指标值字段：">
          <el-input placeholder="请输入指标值字段" v-model="form.valueKey" size="small" maxlength='50'></el-input>
        </el-form-item>
        <el-form-item prop="snKey" label="设备识别码字段：">
          <el-input placeholder="请输入设备识别码字段" v-model="form.snKey" maxlength='50' size="small"></el-input>
        </el-form-item>
        <el-form-item prop="webhook" label="web hook：">
          <el-input placeholder="请输入web hook" v-model="form.webhook" maxlength='50' size="small"></el-input>
        </el-form-item>
        <el-form-item prop="referenceValue" label="安全值：">
          <el-input placeholder="请输入安全值" v-model="form.referenceValue" maxlength='20' size="small"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="resetForm('quota')" size="small" class="butBox">取 消
        </el-button>
        <el-button type="primary" @click="submitQuotaForm('quota')" size="small" class="butBox" style="margin-left:30px">确
          定
        </el-button>
      </div>
    </el-dialog>

    <el-dialog :visible.sync="GPSDialogVisible" width="500px" center>
      <div slot="title" class="dialog-title">
        GPS指标
      </div>

      <el-form :model="gpsForm" :rules="gpsRules" ref="gps" label-width="140px">
        <el-form-item prop="subject" label="报文主题：">
          <el-input placeholder="请输入报文主题" v-model="gpsForm.subject" maxlength='20' size="small"></el-input>
        </el-form-item>
        <el-form-item prop="snKey" label="设备识别码字段：">
          <el-input placeholder="请输入设备识别码字段" maxlength='50' v-model="gpsForm.snKey" size="small"></el-input>
        </el-form-item>
        <el-form-item prop="singleField" label="指标值类型：">
          <el-radio-group v-model="gpsForm.singleField">
            <el-radio :label="true">单字段</el-radio>
            <el-radio :label="false">多字段</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="gpsForm.singleField" prop="valueKey">
          <el-input placeholder="请输入指标值字段（经纬度）" v-model="gpsForm.valueKey" maxlength='20' size="small"></el-input>
        </el-form-item>
        <el-form-item v-if="gpsForm.singleField" prop="separation">
          <el-input placeholder="请输入分隔符" v-model="gpsForm.separation" maxlength='20' size="small"></el-input>
        </el-form-item>
        <el-form-item v-if="!gpsForm.singleField" prop="longitude">
          <el-input placeholder="请输入指标值字段（经度）" v-model="gpsForm.longitude" size="small" maxlength='20'></el-input>
        </el-form-item>
        <el-form-item v-if="!gpsForm.singleField" prop="latitude">
          <el-input placeholder="请输入指标值字段（纬度）" v-model="gpsForm.latitude" size="small" maxlength='20'></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="GPSDialogVisible = false" size="small" class="butBox">取 消
        </el-button>
        <el-button type="primary" @click="submitGPSForm('gps')" size="small" class="butBox" style="margin-left:30px">确
          定
        </el-button>
      </div>
    </el-dialog>
...
<script lang="ts">
  ...
  export default class extends Vue {
    ...
    private form: object = {
      ...
      valueType: "",
      snKey: "",
      webhook: "https://github.com/mikuhuyo",
      referenceValue: ""
    };
